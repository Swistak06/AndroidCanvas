...
class TriangleCanvas(context: Context) : View(context) {
    ...
    override fun onTouchEvent(event: MotionEvent?): Boolean {
        ...
        roznicaX[0] = abs(point13X - event!!.x)
        roznicaX[1] = abs(point23X - event!!.x)
        roznicaX[2] = abs(point31X - event!!.x)
        roznicaY[0] = abs(point13Y - event!!.y)
        roznicaY[1] = abs(point23Y - event!!.y)
        roznicaY[2] = abs(point31Y - event!!.y)
